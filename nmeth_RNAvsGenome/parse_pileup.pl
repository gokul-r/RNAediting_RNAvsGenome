...
sub parse_pileup {
if (@_ < 2) {
	die "need to provide 2 inputs to parse_pileup: pileup line and minbasequal\n"
}
...
LOOP: for (my $j = 0; $j < @pileup; $j++) {  #loop over each individual read and assay the nucleotide called
	...
	if ($indel == 0) {	#count up the occurance of each nucleotide
		if ($pileup[$j] ne '$') {
			...
			if (ord($qualities[$readcount-1]) >= ($minbasequal + 33)) { #make sure base quality is greater than 20 (WATCH OUT THIS IS SANGER format!)
